...
<table class="news_write" >
			...
			<tr>
				...
				<td> <input type="text" id="bTitle" name="bTitle" size = "50" value = "${news.bTitle }"></td>
			</tr>
			...
			<tr>
				<td> 간략정보 </td>
				<td> <textarea name="bSimpContent" id="bSimpContent" rows="20" cols="10">${news.bSimpContent }</textarea> </td>
			</tr>
			...
			<tr>
				<td> 내용 </td>
				<td> <textarea name="bContent" id="bContent" rows="20" cols="100">${news.bContent }</textarea> </td>
			</tr>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			function fn_openNewsList(){
		        ...
		        comSubmit.setUrl("<c:url value='/openNewslist' />");
		        comSubmit.submit();
		    }
			
			function fn_openNewsdelete(){
				var bId = "${news.bId }"
		        var comSubmit = new ComSubmit();//자바스크립트 객체 생성(common.js에 submit기능 함수 만듦)
		        comSubmit.setUrl("<c:url value='/openNewsdelete' />");
		        comSubmit.addParam("bId", bId);
		        comSubmit.submit();
		    }
			
			function fn_openNewsmodify(){
				var bId = "${news.bId }"
		        var comSubmit = new ComSubmit("newsinsert");//자바스크립트 객체 생성(common.js에 submit기능 함수 만듦)
		        comSubmit.setUrl("<c:url value='/openNewsmodify' />");
		        comSubmit.addParam("bId", bId);
		        comSubmit.submit();
		    }
