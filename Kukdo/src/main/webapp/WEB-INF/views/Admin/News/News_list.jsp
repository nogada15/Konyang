...
<body>
<h2>뉴스센터</h2>
<form id="newsmodify" method="post" action="openNewslistserch">
<select name="searchOption">
			<option value="">전체</option>
			<option value="국문">국문</option>
			<option value="영문">영문</option>
			</select>
제목<input name="keyword" value="${news.keyword }">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("a[name='title']").on("click", function(e){//수정페이지
	            e.preventDefault();
	            fn_openNewsmodify($(this));//a태그
	        });
			
		});
		
			function fn_openNewswrite(){
		        var comSubmit = new ComSubmit();//자바스크립트 객체 생성(common.js에 submit기능 함수 만듦)
		        comSubmit.setUrl("<c:url value='/Admin/openNewswrite' />");
		        comSubmit.submit();
		    }
			
			function fn_openNewsmodify(obj){
		        var comSubmit = new ComSubmit();//자바스크립트 객체 생성(common.js에 submit기능 함수 만듦)
		        comSubmit.setUrl("<c:url value='/Admin/openNewsch' />");
